...
| 5.1 kickstart/frozen-feature CV grid search | **Done, now first-class** — was only reachable via `rgb_grains.pipeline`'s `--extra-train-args --run-frozen-features 1` escape hatch; `pipeline.py` now exposes `--run-frozen-features`/`--frozen-feature-c-grid` directly, forwarded to `rgb_grains.train`. |
| `npz_to_jpg` color histogram | **Restored** — consolidating `src/script_npz_to_jpg.py` + `segmentation/control_script_npz_to_jpg.py` into `rgb_grains/utils/npz_to_jpg.py` had silently dropped the per-channel RGB pixel-value histogram diagnostic the original segmentation QC script produced. Re-added as `plot_color_histogram()`, opt-in via `--color-histogram`. |
| Perfomix by-size cleaning threshold | **Still to be studied** — `rgb_grains/data/cleaning.py`'s perfomix `min_area` default is a placeholder (unlike the BACS `110000` criterion, which came from Martin); needs an actual area-distribution study on real perfomix data, not available in this environment. |
